import type { Signer } from 'ethers'
import { ContractFactory } from 'ethers'

const WALLETFACTORY_BYTECODE =
  ''
const WALLETFACTORY_ABI = [
  {
    inputs: [
      { internalType: 'address', name: '_mainModule', type: 'address' },
      { internalType: 'bytes32', name: '_salt', type: 'bytes32' },
    ],
    name: 'deploy',
    outputs: [{ internalType: 'address', name: '_contract', type: 'address' }],
    stateMutability: 'payable',
    type: 'function',
  },
]

export class WalletFactoryContract extends ContractFactory {
  constructor(signer: Signer) {
    super(WALLETFACTORY_ABI, WALLETFACTORY_BYTECODE, signer)
  }
}
